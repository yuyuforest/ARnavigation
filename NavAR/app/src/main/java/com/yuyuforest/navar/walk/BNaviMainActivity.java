package com.yuyuforest.navar.walk;
...
public class BNaviMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        double startLat = getIntent().getDoubleExtra("startLat", 40.057038);
        double startLng = getIntent().getDoubleExtra("startLng", 116.307899);
        double endLat = getIntent().getDoubleExtra("endLat", 40.035916);
        double endLng = getIntent().getDoubleExtra("endLng", 116.340722);

        initMapStatus((startLat + endLat) / 2, (startLng + endLng) / 2);
        ...
        startPt = new LatLng(startLat,startLng);
        endPt = new LatLng(endLat, endLng);
     ...
     * 初始化地图状态
     */
    private void initMapStatus(double lat, double lng){
    ...
    private void startWalkNavi() {
        ...
        try {
            Runnable runnable = new Runnable() {
                @Override
                public void run() {
                    WalkNavigateHelper.getInstance().initNaviEngine(BNaviMainActivity.this, new IWEngineInitListener() {
                        @Override
